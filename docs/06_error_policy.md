...
> 設計契約v1。foundation CLIは成功0・設定不足/失敗1・引数誤り2とJSON出力を実装済み。下表の工程別エラーコード・構造化ログは未実装。正常な品質不良と実行障害を別に扱う。
...
バッチ終了コードは0=処理完了、1=実行失敗を予定する。採否はdecisionに明示し、gateコマンド導入時はaccepted=0、rejected=2、inconclusive=3とする。gate CLIは未実装のため採否の終了コードはまだ利用できない。
